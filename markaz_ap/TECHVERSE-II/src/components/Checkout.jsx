...
import { Alert, Box, Button, Card, Divider, FormControl, InputLabel, MenuItem, Select, TextField } from '@mui/material';
...
const CheckoutPage = () => {
  ...
  console.log(`provinceee:`,province);
  ...
  const totalProfit = Object.values(showItemProfits).reduce((acc, profit) => acc + profit, 0);
  ...
  const handleCheckout = async (event) => {
    ...
    if(!token){
      ...
      <Alert severity="warning">Please Login Your Account For Checkout</Alert>
        ...
        <Grid item xs={8}>
          <div style={{ padding: '20px' }} className='child1'>
            ...
            <form onSubmit={handleCheckout}>
              ...
              <div style={{ display: 'flex', justifyContent:'center' , gap:'1rem', padding:'10px' }}>

                <Button variant='contained' type='submit' onClick={notify} >Checkout</Button>
                    ...
                    <Box>
                      <Box key={index} sx={{ display: "flex", justifyContent: "space-evenly", py: 1 }}>
                        <Box>
                          ...
                          <img src={img7} style={{ borderRadius: '5px', height: '100px' }} alt="ProductImage" />
                        </Box>
                        <Box>
                          ...
                          <Button variant="outlined" onClick={() => { dispatch(incrementQ(item)); handleIncQuantity(item) }}>
                            +
                          </Button>
                          <Button variant="outlined" onClick={() => { dispatch(decrementQ(item)); handleDecQuantity(item) }}>
                            -
                          </Button>
                        </Box>
                        <Box>
                          <Box pb={2} >
                            <p>My Profit: </p>
                            <TextField value={itemProfits[item._id] || 0} onChange={(event) => calculateItemProfit(item, parseFloat(event.target.value))} />
